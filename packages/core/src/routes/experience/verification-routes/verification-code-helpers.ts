 ...
 * - Sign-in from an unidentified session to an identifier no user owns when sign-up via that
 *   identifier is disabled (behind `isDevFeaturesEnabled`); identified sessions always deliver.
 */
const shouldSkipDelivery = async (
  ...
  if (
    ...
    const isSignUpEnabled =
      await experienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled(
        identifier.type
      );

    return !isSignUpEnabled && !(await hasUserWithIdentifier(queries, identifier));
  }
