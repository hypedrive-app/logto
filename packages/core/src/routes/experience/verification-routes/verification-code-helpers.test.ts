...
type MockExperienceInteraction = {
  ...
  signInExperienceValidator: {
    ...
    isSignUpIdentifierEnabled: jest.MockedFunction<() => Promise<boolean>>;
  };
};
...
describe('sendCode parameter passing', () => {
  ...
  const mockExperienceInteraction: MockExperienceInteraction = {
    ...
    signInExperienceValidator: {
      ...
      // Default: sign-up via the identifier is enabled, so delivery is never suppressed for sign-in.
      isSignUpIdentifierEnabled: jest.fn().mockResolvedValue(true),
    },
  };
  ...
  beforeEach(() => {
    ...
    // Restore the default: sign-up via the identifier is enabled, so sign-in delivery is not suppressed.
    mockExperienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled.mockResolvedValue(
      true
    );
  });
  ...
  it('skips delivery for sign-in to an unknown recipient when sign-up via the identifier is disabled', async () => {
    mockExperienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled.mockResolvedValueOnce(
      false
    );
  ...
  it('delivers for sign-in to an unknown recipient when sign-up via the identifier is enabled', async () => {
    mockExperienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled.mockResolvedValueOnce(
      true
    );
    const ctx = buildSignInCtx();
  ...
  it('delivers for sign-in binding when a user is already identified, even for an unknown recipient with sign-up disabled', async () => {
    // Binding a new MFA identifier: identified session, unknown recipient, identifier not a sign-up one.
    mockExperienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled.mockResolvedValueOnce(
      false
    );
    ...
    // The identified-session guard short-circuits before consulting sign-up settings.
    expect(
      mockExperienceInteraction.signInExperienceValidator.isSignUpIdentifierEnabled
